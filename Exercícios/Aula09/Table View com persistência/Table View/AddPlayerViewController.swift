...
class AddPlayerViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        guard let nome = self.nomeTF.text,
              ...
              let tableVC = segue.destination as? PlayerTableViewController,
              let context = tableVC.context
            else {
                fatalError("salvando sem preencher os campos")
        }
        
        if let numero = Int(numeroString) {
            self.newPlayer = PlayerMO(context: context)
            self.newPlayer?.nome = nome
            self.newPlayer?.posicao = posicao
            self.newPlayer?.numero = Int32(numero)
            self.newPlayer?.foto = NSData(data: UIImageJPEGRepresentation(foto, 1)!)
        }
    }
}
